...
// ServiceDetailSection component remains the same...
const ServiceDetailSection = ({ service, index }) => {
  ...
  useEffect(() => {
    if (sectionRef.current) {
      ...
      gsap.fromTo(contentEl, { autoAlpha: 0, x: isReversed ? -50 : 50 }, { autoAlpha: 1, x: 0, duration: 0.8, ease: 'power2.out', delay: 0.2, scrollTrigger: { trigger: sectionRef.current, start: "top 80%" }});
    }
  }, [isReversed]);
...
const ServicesPage = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 3D Mouse Move Parallax Effect for Header
  ...
  useEffect(() => {
      const ctx = gsap.context(() => {
        // General reveal for sections below the header
      ...
      <div ref={mainContentRef} className="bg-white">
        ...
        <section ref={pageHeaderRef} className="relative section-padding text-white text-center overflow-hidden">
          <div 
            ...
            style={{ backgroundImage: `url('/assets/services-background.jpg')`, transform: 'scale(1.1)' }}
          >
